...
interface ColoredSegment { text: string; fg: string }

/** github-dark-dimmed class → hex color mapping */
const HLJS_COLORS: Record<string, string> = {
  "hljs-keyword": "#f47067", "hljs-doctag": "#f47067",
  "hljs-template-tag": "#f47067", "hljs-template-variable": "#f47067",
  "hljs-type": "#f47067", "hljs-variable language_": "#f47067",
  "hljs-title": "#dcbdfb", "hljs-title class_": "#dcbdfb",
  "hljs-title function_": "#dcbdfb",
  "hljs-attr": "#6cb6ff", "hljs-attribute": "#6cb6ff",
  "hljs-literal": "#6cb6ff", "hljs-meta": "#6cb6ff",
  "hljs-number": "#6cb6ff", "hljs-operator": "#6cb6ff",
  "hljs-variable": "#6cb6ff", "hljs-selector-attr": "#6cb6ff",
  "hljs-selector-class": "#6cb6ff", "hljs-selector-id": "#6cb6ff",
  "hljs-regexp": "#96d0ff", "hljs-string": "#96d0ff",
  "hljs-built_in": "#f69d50", "hljs-symbol": "#f69d50",
  "hljs-comment": "#768390", "hljs-code": "#768390",
  "hljs-formula": "#768390",
  "hljs-name": "#8ddb8c", "hljs-quote": "#8ddb8c",
  "hljs-selector-tag": "#8ddb8c", "hljs-selector-pseudo": "#8ddb8c",
  "hljs-section": "#316dca", "hljs-bullet": "#eac55f",
  "hljs-addition": "#b4f1b4", "hljs-deletion": "#ffd8d3",
};
...
interface InlineSegment { text: string; fg: string; bg?: string }
...
function isKey(
  ...
  if (modifier === "ctrl" && sequence === String.fromCharCode(code & 0x1f))
    return true;
  if (
    modifier === "alt" &&
...
const conversationsPanel = Box(
  ...
  Box(
    ...
    Text({
      content: "Select a conversation (q to go back):",
  ...
  Select({
    ...
    focusedBackgroundColor: THEME.bgMain,
    ...
    options: [
      { name: "+ New Conversation", description: "Create a new chat" },
    ],
  })
);
...
const modelsPanel = Box(
  ...
  Box(
    ...
    Text({
      content: "Select a model (q to go back):",
      fg: THEME.textPrimary,
    })
  ),
  Select({
...
function showMode(newMode: "chat" | "conversations" | "models") {
  ...
  if (newMode === "chat") {
    if (statusText)
      statusText.content =
        "Alt+C: conversations | Alt+M: models | Ctrl+C: quit";
    const input = findById<InputRenderable>(renderer.root, "chat-input");
    if (input) input.focus();
  } else if (newMode === "conversations") {
    if (statusText)
      statusText.content =
        "Enter: select | q: back | Alt+C: back | Ctrl+C: quit";
    const sel = findById<SelectRenderable>(
      renderer.root,
      "conversation-select"
    );
    if (sel) sel.focus();
    ...
    if (statusText)
      statusText.content =
        "Enter: select | q: back | Alt+M: back | Ctrl+C: quit";
...
function renderMessages() {
  const scroll = findById<ScrollBoxRenderable>(
    renderer.root,
    "messages-scroll"
  );
  if (!scroll) return;
  ...
  for (let i = 0; i < messages.length; i++) {
    ...
    const modelTag =
      !isUser && msg.model ? ` [${getModelName(msg.model)}]` : "";
    ...
    if (isUser) {
      ...
      if (i > 0) {
        scroll.add(
          Text({ id: `spacer-${i}`, content: "", height: 1, width: "100%" })
        );
      }
      ...
      if (segments.length <= 1 && segments[0]?.type !== "code") {
        ...
        for (let s = 0; s < segments.length; s++) {
          ...
          if (seg.type === "text") {
            ...
            if (langLabel) {
              codeChildren.push(
                Text({ content: langLabel, fg: THEME.textMuted })
              );
            }
...
async function selectConversation(conversationId: Id<"conversations">) {
  ...
  if (conv) {
    ...
    if (modelLabel)
      modelLabel.content = `Model: ${getModelName(selectedModel)}`;
...
setTimeout(() => {
  ...
  // Conversation select
  const convSel = findById<SelectRenderable>(
    renderer.root,
    "conversation-select"
  );
  if (convSel) {
    convSel.on(SelectRenderableEvents.ITEM_SELECTED, () => {
      ...
      if (selected.value === "__new__") {
        ...
        messages = [];
        showMode("chat");
        const convLabel = findById<TextRenderable>(
          renderer.root,
          "conv-label"
        );
  ...
  if (modSel) {
    modSel.on(SelectRenderableEvents.ITEM_SELECTED, () => {
      ...
      const modelLabel = findById<TextRenderable>(
        renderer.root,
        "model-label"
      );
      if (modelLabel)
